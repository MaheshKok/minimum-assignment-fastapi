...
def upgrade() -> None:
    ...
    op.create_table('air_travel_activities',
    sa.Column('distance_miles', sa.Numeric(precision=10, scale=2), nullable=False, comment='Distance traveled in miles'),
    sa.Column('distance_km', sa.Numeric(precision=10, scale=2), nullable=False, comment='Distance traveled in kilometres (converted from miles)'),
    sa.Column('flight_range', sa.String(length=50), nullable=False, comment='Flight range category (e.g., Short-haul, Long-haul, International)'),
    sa.Column('passenger_class', sa.String(length=50), nullable=False, comment='Passenger class (e.g., Economy, Business, First)'),
    ...
    sa.Column('source_file', sa.String(length=255), nullable=True, comment='Original CSV filename if imported from file'),
    sa.Column('raw_data', sa.JSON(), nullable=True, comment='Original CSV row data for audit trail'),
    ...
    op.create_index(op.f('ix_air_travel_activities_date'), 'air_travel_activities', ['date'], unique=False)
    ...
    op.create_index(op.f('ix_electricity_activities_country'), 'electricity_activities', ['country'], unique=False)
    ...
    sa.Column('lookup_identifier', sa.String(length=200), nullable=False, comment="Identifier used to match activity data (e.g., 'United Kingdom', 'Long-haul, Business class')"),
    sa.Column('unit', sa.String(length=50), nullable=False, comment='Unit of measurement (e.g., kWh, GBP, kilometres)'),
    sa.Column('co2e_factor', sa.Numeric(precision=10, scale=6), nullable=False, comment='CO2e emission factor value (kgCO2e per unit)'),
    sa.Column('scope', sa.Integer(), nullable=False, comment='GHG Protocol scope (1, 2, or 3)'),
    sa.Column('category', sa.Integer(), nullable=True, comment='GHG Protocol Scope 3 category number (for Scope 3 only)'),
    sa.Column('source', sa.String(length=200), nullable=True, comment="Source of the emission factor (e.g., 'DEFRA 2024', 'EPA 2023')"),
    sa.Column('notes', sa.String(), nullable=True, comment='Additional notes or context about this emission factor'),
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    sa.PrimaryKeyConstraint('id'),
    comment='Emission factor lookup table for CO2e calculations'
    )
    op.create_index('ix_emission_factors_activity_lookup', 'emission_factors', ['activity_type', 'lookup_identifier'], unique=False)
    ...
    sa.Column('description', sa.String(), nullable=True, comment='Additional description of the purchase'),
    sa.Column('id', sa.UUID(), nullable=False),
    sa.Column('date', sa.Date(), nullable=False, comment='Date when the activity occurred'),
    sa.Column('activity_type', sa.String(length=100), nullable=False, comment='Type of activity'),
    sa.Column('source_file', sa.String(length=255), nullable=True, comment='Original CSV filename if imported from file'),
    ...
    op.create_index(op.f('ix_goods_services_activities_date'), 'goods_services_activities', ['date'], unique=False)
    ...
    sa.Column('activity_id', sa.UUID(), nullable=False, comment='ID of the specific activity record'),
    sa.Column('emission_factor_id', sa.UUID(), nullable=False, comment='Emission factor used in the calculation'),
    sa.Column('co2e_tonnes', sa.Numeric(precision=15, scale=7), nullable=False, comment='Calculated CO2e emissions in tonnes'),
    sa.Column('confidence_score', sa.Numeric(precision=3, scale=2), nullable=False, comment='Confidence score for emission factor matching (0.0 to 1.0)'),
    sa.Column('calculation_metadata', sa.JSON(), nullable=True, comment='Additional calculation details (method, intermediate values, etc.)'),
    sa.Column('calculation_date', sa.Date(), nullable=False, comment='Date when the emission was calculated'),
    sa.Column('created_at', sa.DateTime(), nullable=False),
    sa.Column('updated_at', sa.DateTime(), nullable=False),
    ...
    op.create_index('ix_emission_results_activity', 'emission_results', ['activity_type', 'activity_id'], unique=False)
    op.create_index('ix_emission_results_calculation_date', 'emission_results', ['calculation_date'], unique=False)
    op.create_index('ix_emission_results_created_desc', 'emission_results', ['created_at'], unique=False)
...
def downgrade() -> None:
    ...
    op.drop_index(op.f('ix_goods_services_activities_supplier_category'), table_name='goods_services_activities')
    op.drop_index('ix_goods_services_activities_date_desc', table_name='goods_services_activities')
    op.drop_index('ix_goods_services_activities_date_category', table_name='goods_services_activities')
